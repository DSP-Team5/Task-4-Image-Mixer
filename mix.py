...
class MainApp(QMainWindow, MAIN_WINDOW):
    def __init__(self, parent=None):
        ...
        self.images = {}
        self.image_Component_Windows = [
            self.viewFirstComponent, self.viewSecondComponent]
        self.OutputWindows = {
            "Output 1": self.viewOutput1, "Output 2": self.viewOutput2}
        self.Windows_List = [self.viewFirstimg, self.viewSecondimg, self.viewOutput1,
                             self.viewOutput2, self.viewFirstComponent, self.viewSecondComponent]
        self.ImageModels = {}
        self.output_comboBox = [self.comboBox_4, self.comboBox_5]
        self.mix_comp = ['Magnitude', 'Phase', 'Real',
                         'Imaginary', 'UniMagnitude', 'UniPhase']
        ...
        self.mix_comboBox = [self.comboBox_3, self.comboBox_4,
    ...
    def browse(self, number):
        ...
        fileName = QtWidgets.QFileDialog.getOpenFileName(
            None, "QFileDialog.getOpenFileName()", "", "Image files (*.jpg)")
        ...
        if number == 1:
            if ((self.width[number] != self.width[number-1]) & (self.height[number] != self.height[number-1])):
                ...
                pass
        self.images['Image'+str(number+1)
                    ] = cv2.imread(self.imagePath[number], 0)
        self.updateCombosChanged(self.images['Image'+str(number+1)].T, number)
        self.displayImage(
            self.images['Image'+str(number+1)].T, self.imageWindows[number])
        self.ImageModels['Image'+str(number+1)
                         ] = ImageModel(self.imagePath[number], number)
        self.img_comboBox[number].currentIndexChanged.connect(
            lambda: self.ComboBox_function(number))
    ...
    def updateCombosChanged(self, data, number):
        ...
        self.component_list['Image'+str(number+1) +
                            '_list'] = [Magnitude, phase, real, imaginary]
    ...
    def ComboBox_function(self, number):
        for i in range(4):
            if self.img_comboBox[number].currentText() == self.mix_comp[i]:
                self.displayImage(
                    self.component_list['Image'+str(number+1)+'_list'][i].T, self.image_Component_Windows[number])

    def valuechange(self):
        for i in range(2):
            self.ratio[i] = self.sliders[i].value() / 100.0
        mode = self.comboBox_6.currentText()+str('And')+self.comboBox_7.currentText()
        self.combobox_update(self.comboBox_6.currentText(),
        ...
        if self.comboBox_6.currentText() != 'Select Component' and self.comboBox_7.currentText() != ('Select A Component'):
            logger.info('Current mode is {} and {}' . format(
                self.comboBox_6.currentText(), self.comboBox_7.currentText()))
            logger.info('mixing {} of {} with {} of {} ' . format(self.sliders[0].value(
            ), self.comboBox_6.currentText(), self.sliders[1].value(),  self.comboBox_7.currentText()))

        mixInverse = self.ImageModels[self.comboBox_4.currentText()].mix(
            self.ImageModels[self.comboBox_5.currentText()], self.ratio[0], self.ratio[1], mode)
        logger.info('Component 1 is {} , component 2 is {}' . format(
            self.comboBox_4.currentText(), self.comboBox_5.currentText()))
        self.displayImage(
            mixInverse, self.OutputWindows[self.comboBox_3.currentText()])
